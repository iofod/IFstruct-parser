...
function routerGo(param, next) {
  window.FN.PS.publish('Fx_router_go', {
    param
  })
...
function setModel(data, next) {
  ...
  if (typeof target != 'string') return warn(target, 'is invalid')
  if (typeof key != 'string') return warn(key, 'is invalid')
...
function useInteractionFlow(data, next) {
  ...
  window.FN.PS.publishSync('Fx_interactionFlow', data)
...
function useInterpolation(data, next) {
  ...
  window.FN.PS.publishSync('Fx_interpolation', data)
...
function editStatu(data, next) {
  window.FN.PS.publishSync('Fx_editStatu', data)

  next('editStatu done!')
}

function setCPA(data, next) {
  window.FN.PS.publishSync('Fx_setCPA', data)
