...
function genEventContent(hid, events, cloneMark, jumpCE = true) {
  ...
  events.forEach((evo) => {
    ...
    ;['passive', 'capture', 'once', 'prevent', 'stop', 'self'].forEach(
      (key) => {
        if (evo[key]) {
          prefix += '.' + key
        }
...
function genTag(hid, tag) {
  const flag = IF.ctx.HSS[hid].status.filter(
...
function genChildView(hid, IN = '', DI = 0, eventContent) {
  const target = tree[hid] || IF.ctx.HSS[hid]
...
function genCreated(lid) {
  ...
  CE_list.forEach((evo, I) => {
    ...
    const sub_name =
      event == 'modelchange' ? `${target || hid}.${mds}.${event}` : event
...
function genViewContent(lid, payload) {
  ...
  const { eventMarks, eventMethods } = genEventContent(
    lid,
    tree[lid].events,
    'clone'
  )
